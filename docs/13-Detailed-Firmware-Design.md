...
The Stage 3A.1 text-resource ABI is intentionally stricter than the future sparse
font/image resource catalog. Text tables use dense IDs `0x0001..0x0038`; firmware
validates payload CRC, header fields, dense record order, index CRC, UTF-8, record
bounds, and exact blob consumption for both required languages before allowing language
selection. The hot resolver retains only the selected language metadata and performs a
direct indexed record read.
