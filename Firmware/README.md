...
Runtime rendering uses fixed small scratch buffers and stable asset IDs. A typical transfer sequence is: select W25Q, read a chunk, release Flash CS, select TFT CS, transmit/render that chunk, release TFT CS, and continue. Installed font/resource size must not create proportional SRAM usage; only renderer state, a bounded shared resource buffer, and perhaps a tiny metadata cache belong in SRAM.

The application layer also owns one explicit 3072-byte I/O workspace. Phase 05 raw
metrology capture and calibration-store frame serialization share this arena through
exclusive ownership tags, so the firmware does not reserve one full raw ADC buffer and
one full calibration frame at the same time.

Resource Pack v2 API version 3 uses dense UTF-8 product text catalogs plus three
external A1 bitmap font role resources for the current PRODUCT UI. Firmware validates
both English and Portuguese (Brazil) text resources before admitting a pack: payload
CRC, text-table header, dense `0x0001..0x0038` ID coverage, index CRC, record bounds,
exact blob use, and UTF-8. Firmware also validates `FONT_UI_SMALL`, `FONT_UI_MEDIUM`,
and `FONT_UI_LARGE` as `FONT_BITMAP_A1_V1`: a 32-byte `WFA1` header, sorted 20-byte
glyph records, and bounded row-major MSB-first bitmaps. Fatal normal text/font
resource errors enter the PRODUCT `RESOURCE_ERROR` presentation; `RESOURCE_STATUS_DEFERRED`
remains retry/backpressure for quiet mode or W25Q mutation.
