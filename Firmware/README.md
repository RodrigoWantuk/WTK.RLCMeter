...
Resource Pack v2 API version 2 uses dense UTF-8 product text catalogs for the current
fallback-renderer contract. Firmware validates both English and Portuguese (Brazil)
text resources before admitting a pack: payload CRC, text-table header, dense
`0x0001..0x0038` ID coverage, index CRC, record bounds, exact blob use, and UTF-8.
Fatal normal text-resource errors enter the PRODUCT `RESOURCE_ERROR` presentation;
`RESOURCE_STATUS_DEFERRED` remains retry/backpressure for quiet mode or W25Q mutation.
